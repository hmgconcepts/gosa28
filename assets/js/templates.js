...
<script>
  ...
  roleAllow(id) {
    ...
    const map = {
      // Safe common pages (always available)
      dashboard: allUsers, notifications: allUsers, 'verify-certificate': publicPages, 'teacher-overview': admin, 'feature-guide': allUsers, about: publicPages, contact: publicPages, apply: publicPages,
      announcements: allUsers, events: allUsers, school_calendar: allUsers, voting: allUsers, gallery: allUsers,
      ...
      activity_log: admin, storage: admin, settings: admin, promotion: admin, alumni: admin, financial_aid: admin, donations: admin, broadcast: admin, visitors: admin,
      front_desk: admin, document_builder: admin, fleet_tracking: admin, facility_booking: admin, transport: admin, hostel: admin, menu: admin, cafeteria: admin, leave: admin,

      // Staff/teacher operational pages
      students: staff, classes: staff, subjects: staff, academic_records: staff, 'academic-records': staff,
      cbt: staff, cbt_prompts: staff, entrance: staff, timetable_generator: staff, 'timetable-generator': staff, sow: staff, lesson_plans: staff, reports: staff, directory: staff, rubrics: staff, substitutions: staff, book_request: staff, library: staff, transfer_cert: staff, checkin: staff, career_counseling: staff, behaviour: staff, support_plans: staff, conduct: staff, health: staff, parent_meeting: staff, idcards: staff, certificates: staff, transcripts: staff, diary: staff, surveys: staff, lost_found: staff, helpdesk: staff, eresources: staff, messages: staff, inbox: staff, complaints: staff, counselling: staff, developer: staff,

      // Shared pages for parents & students
      attendance: allUsers, results: allUsers, report_cards: allUsers, 'report-cards': allUsers, student_profile: allUsers, 'student-profile': allUsers,
      timetable: allUsers, assignments: allUsers, digital_library: allUsers,
      fees: admin + ' ' + parent, payments_online: admin + ' ' + parent,
      lms: admin + ' ' + staff + ' ' + learner, gamification: admin + ' ' + staff + ' ' + learner, cbt_exam: publicPages, 'cbt-exam': publicPages
    };
    const aliases = { class:'classes', subject:'subjects', result:'results', student:'students',
                      staff:'staff', parent:'parents', promotion:'promotion' };
    return map[id]
      || (id.includes('-') ? map[id.replace(/-/g, '_')] : undefined)
      || (aliases[id] ? map[aliases[id]] : undefined)
      || staff;
  },
  ...
  allModules(config) {
    ...
    const dedicatedPages = [
      'student-profile', 'cbt-exam', 'verify-certificate', 'teacher-overview',
      'feature-guide', 'about', 'contact', 'apply', 'notifications',
      // FIX v9: Parents page was missing from nav
      'parents'
    ];
    // Combine and dedupe — avoid 'class' vs 'classes' collisions
    const allIds = [...new Set([...base, ...selected, ...catalogIds, ...dedicatedPages])].filter(Boolean);
    ...
    return T.shell(config, 'Dashboard', `
      ...
      <section id="dash-sec-parent" data-dash-role="parent" style="display:none">
        ...
        <div class="card" style="margin-bottom:16px;background:#f0fdf4;border-color:#bbf7d0">
          <h3 style="margin-top:0">👨‍👩‍👧 My Children's Dashboard</h3>
          <p style="color:var(--gray-700)">Access the complete 360-degree academic dashboard for your linked children (report cards, results, attendance, timetable, assignments, fees).</p>
          <div id="dash-parent-children-list" style="margin-top:12px"><span class="pulse">Fetching linked children…</span></div>
        </div>
    ...
    return T.shell(config, 'Student Profile', `
      ...
      const StudentProfile = {
        ...
        async loadChildren(parentId) {
          ...
          if (!container || !sb) return;
          // First get linked students via parent_student table
          const { data: links } = await sb.from('parent_student').select('student_id').eq('parent_id', parentId).catch(()=>({data:[]}));
  ...
  modulePage(config, moduleId, opts = {}) {
    ...
    const readOnly = false; // Admin always gets read/write permission for every feature
    ...
    const adminOnlyModules = ['academic_setup','departments','admissions','approvals','admin_data','admin-data',
      ...
      'parent_student','cbt_prompts','timetable_generator','rubrics','transcripts','transfer_cert',
    ...
    return T.shell(config, mod.name, `
      ...
      document.addEventListener('DOMContentLoaded', function() {
        ...
        // Load the CRUD list for staff/admin
        if (isAdminOrStaff && window.CRUD) {
...
window.T = T;
/* Critical app-shell CSS (inlined in <head> for bulletproof rendering) */
